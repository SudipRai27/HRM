...
<script type="text/javascript">
	$(document).ready(function(){
		...
		{			
			$('#leave-results').html('<p align="center"><img src = {{asset('public/images/giphy.gif')}}></p>')
			$.ajax({
				'method' : 'GET', 
				'data' : {
				...
				'url' : '{{route('ajax-get-leave-history-from-date-and-user-id')}}'

			}).done(function(data){
				$('#leave-results').html(data);
			});
